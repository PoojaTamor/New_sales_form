...
$(".btn-next").click(function () {
  ...
  const phoneInput = current_fs.find("#phoneNumber").val();
  const phoneRegex = /^[0-9]{10}$/; // Sirf 10 digits
  if (phoneInput) {
...
value="${form.SaleSource?.value === 'MarketingPerson' ? (form.MarketingPersonName?.value || '') : (form.SaleSource?.value || '')}" 
    disabled>
  </div>
</div>
...
$(document).on("click", "#confirmSubmit", function () {
  ...
  fetch("https://script.google.com/macros/s/AKfycbxzLB1ftoj7xkLRM8frdBWpKHrc06yR1tIutNVY6ss4G-TEHl0fa47sA1VJdLeekLbmzg/exec", {
method: "POST",
  mode: "cors", 
  redirect: "follow", 
  headers: {
...
//   let marketingInput = document.getElementById("MarketingPersonName");
//   if (this.value === "MarketingPerson") {
//     marketingInput.style.display = "block";
//     marketingInput.required = true; // Jab dikhe tab required
//   } else {
//     marketingInput.style.display = "none";
//     marketingInput.required = false;
//   }
// });
...
// Common function
function toggleInput(selectId, conditions) {
  const selectEl = document.getElementById(selectId);

  selectEl.addEventListener("change", function () {
    // sab inputs ko hide kar do
    Object.values(conditions).forEach((id) => {
    ...
    // ab sirf us value ka input dikhao jo select hua
    if (conditions[this.value]) {
      let activeInput = document.getElementById(conditions[this.value]);
